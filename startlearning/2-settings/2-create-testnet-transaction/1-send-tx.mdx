...
When creating a transaction on the XPLA blockchain, you need to pay fees in XPLA coins. Since you received testnet XPLA coins in the [previous step](/startlearning/settings/create-wallet/get-testnet-xpla), you can create transactions on the testnet without worrying about fees.
...
If you've understood the Preview Code, you may move on to the [next step](/startlearning/settings/create-testnet-transaction/check-tx-in-explorer) with out reading [the below](/startlearning/settings/create-testnet-transaction/send-tx#runjs).
