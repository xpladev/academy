...
예제처럼 초깃값을 설정하면, 발행한 CW20 토큰의 기본 단위는 **MCT**입니다. 그런데 때로는 더 작은 단위가 필요할 수도 있습니다. CW20의 최소 단위는 기본 단위에 접두사로 **u**를 추가합니다. 예제 컨트랙트는 **uMCT**가 되겠군요.

예제 초깃값에서 `decimals` 항목을 **6**으로 지정했기 때문에 10<sup>6</sup> **uMCT** = 1 **MCT** 가 됩니다. 참고로 **XPLA**는 decimals는 **18**이므로, 10<sup>18</sup> **aXPLA** = 1 **XPLA** 였습니다. 
