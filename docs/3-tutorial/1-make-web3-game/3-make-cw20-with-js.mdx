...
# Javascript로 게임 토큰(CW20) 발행 및 전송하기
...
CW20을 이용하면 게임 생태계와 XPLA 블록체인을 더 쉽게 연결할 수 있습니다. 이번에는 직접 코드를 작성하여 게임 토큰을 발행해봅시다.
...
Preview Code를 이해하셨다면, [아래 내용](/docs/tutorial/make-web3-game/make-cw20-with-js#make-cw20)을 읽지 않고 바로 [다음 단계](/docs/tutorial/add-func-to-game-1/overview)로 넘어가셔도 좋습니다.
...
위 코드에서 `decimals` 항목을 **6**으로 지정했기 때문에 10<sup>6</sup> **uMCT** = 1 **MCT** 가 됩니다. 참고로 **XPLA**는 decimals는 **18**이므로, 10<sup>18</sup> **aXPLA** = 1 **XPLA** 였습니다. 
...
3. 10번째 줄의 니모닉 단어들을 여러분의 니모닉 단어로 수정해주세요. 
  ...
  여러분이 [Javascript로 CW20 발행하기 단계](/docs/tutorial/make-web3-game/make-cw20-with-js#make-cw20)에서 생성한 컨트랙트 주소를 16번째 줄의 `cw20_contract` 변수에 대입해주세요. 예제에서 `xpla1xljvdrtyn86kv7hrdhae4qxdy8krajah3w7xhtyrt0n69und9xdqdhrasc`를 여러분의 컨트랙트 주소로 대체하시면 됩니다.
  ...
  마지막으로 CW20 토큰을 받게 될 지갑 주소를 17번째 줄의 `recipient` 변수에 대입해주세요. 예제에서 `xpla1cwduqw0z8y66mnfpev2mvrzzzu98tuexepmwrk`를 받게 될 지갑 주소로 대체하시면 됩니다.
...
토큰의 컨트랙트 주소와, 토큰을 받게 될 지갑 주소를 지정해주었습니다. 여러분들도 직접 만든 토큰의 컨트랙트 주소와 지갑 주소를 대입하셨을 것입니다.
...
실행할 컨트랙트 주소, 컨트랙트에서 사용할 함수, 함수의 매개변수값을 입력하여 컨트랙트 실행을 준비합니다. CW20을 얼만큼 보낼지도 지정할 수 있습니다. `amount` 항목에 수량을 지정하는데, 단위는 uCW20입니다. [앞서 컨트랙트를 생성할 때](/docs/tutorial/make-web3-game/make-cw20-with-js#make-cw20-explaination), `decimal` 항목이 **6**이었다는 점을 생각해봅시다. **uCW20**이 100000개라면, **CW20** 0.1개를 전송하겠군요.
...
`createAndSignTx` 함수는 트랜잭션을 생성하고, 수수료 금액을 자동으로 설정해주고, 서명도 진행합니다. [Javascript로 CW20 발행하기 단계](/docs/tutorial/make-web3-game/make-cw20-with-js#make-cw20-explaination)에서 이를 각각 분리한 코드도 함께 보여드렸습니다.
