...
3. **Dockerfile**을 Build하여 Docker Image를 생성합니다. Build 하는 데 시간이 걸릴 수 있습니다.
    ...
    `docker image ls` 스크립트를 입력하여 Docker Image가 잘 생성되었는지 확인해봅시다.
...
4. 생성한 Docker Image로 Docker Container를 생성합니다.
    ...
    `docker ps -a` 스크립트를 입력하여 Docker Container가 잘 생성되었는지 확인해봅시다.
...
12. 지금까지 설정한 Genesis 파일 상태대로 블록체인 데이터베이스를 초기화해줍니다.
...
지금까지 xplad를 활용하여 Local Network 블록체인이 잘 동작하는 것을 확인하였습니다. 더 많은 xplad 명령어는 [Docs](https://docs.xpla.io/docs/develop/tools/xplad/commands/)를 참고해보시거나, `xplad -h` 스크립트를 입력해보세요.    
