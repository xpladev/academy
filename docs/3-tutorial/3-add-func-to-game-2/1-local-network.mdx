...
1. [Prerequisite](/docs/tutorial/add-func-to-game-2/local-network#prerequisite)

2. [Local Network 실행](/docs/tutorial/add-func-to-game-2/local-network#run-local-network)

3. [Local Network에서 xplad 사용하기](/docs/tutorial/add-func-to-game-2/local-network#use-xplad)

4. [RPC 네트워크 이용하기](/docs/tutorial/add-func-to-game-2/local-network#use-rpc)

5. [Swagger 이용하기](/docs/tutorial/add-func-to-game-2/local-network#use-swagger)

6. [Javascript로 Local Network에 트랜잭션 생성하기](/docs/tutorial/add-func-to-game-2/local-network#use-js)
...
3. **Dockerfile**을 Build하여 Docker Image를 생성합니다. Build 하는 데 시간이 걸릴 수 있습니다.

    ```bash
    docker build -t my-xpla-node:20220819 .
    ```

    생성한 Docker Image를 확인할 수 있는 스크립트는 `docker image ls` 입니다.

4. 생성한 Docker Image로 Docker Container를 생성합니다.

    ```bash
    docker run -it -d --name myXPLANode -p 26657:26657 -p 1317:1317 my-xpla-node:20220819
    ```

    생성한 Docker Container를 확인할 수 있는 스크립트는 `docker ps -a` 입니다.
...
5. Docker Container에 bash로 접속합니다. 
    ...
    혹시 예제 과정을 따라하다 놓친 부분이 있다면, Container를 삭제 후 다시 생성하는 방법이 있습니다. `docker stop myXPLANode && docker rm myXPLANode` 스크립트로 Container를 삭제 후, 4번 과정부터 다시 진행해보세요.

    :::

6. Docker Container의 bash 환경에서 아래 스크립트를 입력합니다. Mainnet의 chainId가 `dimension_37-1`인 것처럼, 예제의 Local Network에서는 chainId를 `localnet_1-1`로 설정하였습니다.  **yourMonikerName** 변수는 여러분이 운영하는 노드의 이름을 의미합니다. 여러분이 원하는 이름으로 입력해주세요. 

    ```bash
    xplad init --chain-id=localnet_1-1 yourMonikerName
    ```

    위 스크립트를 입력하게 되면 home 폴더(~)에 `.xpla` 폴더가 생성되고 Genesis 파일을 초기화하게 됩니다. 
    ...
    [Mainnet](https://github.com/xpladev/mainnet/tree/main/dimension_37-1)과 [Testnet](https://github.com/xpladev/testnets/tree/main/cube_47-5)도 Genesis 파일이 있고, 모두 Github에서 확인하실 수 있습니다. 만약 Mainnet과 동일한 Genesis 파일을 사용한다면, 여러분도 Mainnet [Full Node(블록체인의 모든 정보를 저장하고 있는 노드)](https://docs.xpla.io/docs/full-node/run-a-full-node/overview/)를 운영할 수 있습니다. 예제에서는 Mainnet도, Testnet도 아닌, Local Network를 운영할 것이기 때문에 직접 Genesis 파일을 구성하여 사용하였습니다.

    Mainnet과 Testnet의 Full Node를 운영하는 더 자세한 방법은 [Docs](https://docs.xpla.io/docs/full-node/run-a-full-node/join-a-network/#join-a-public-network)를 참고해보세요.

    </Details>

7. Local Network 블록체인에서 Validator로 사용할 지갑을 생성합니다. [이전 단계](/docs/settings/create-wallet/create-wallet-with-xplad)에서도 동일한 스크립트로 지갑을 생성하였습니다.
...
8. 생성한 지갑에 100000000000000000000axpla(100 XPLA)만큼 지급한다는 내용을 `~/.xpla/config/genesis.json` 파일에 기록합니다.
    ...
    Genesis 블록에서 지갑 하나에 100 XPLA를 지급하므로, Local Network의 XPLA 총 발행량은 100 XPLA입니다.
...
9. 생성한 지갑을 검증자(Validator)로 등록합니다. 10000000000000000000axpla(10 XPLA)는 검증자로 등록할 때, Self-Delegate할 초기 물량입니다.
    ...
    첫번째 스크립트를 입력하면 `~/.xpla/config`에 `gentx` 폴더가 추가로 생성됩니다. 두번째 스크립트를 입력하면 `gentx` 폴더의 내용이 `genesis.json` 파일에 반영됩니다.
...
10. 현재 `genesis.json` 파일에서 코인 단위가 stake로 설정되어 있습니다. XPLA 블록체인에서 사용하는 코인 단위는 `axpla`이므로 수정해줍니다.

    ```bash
    sed -i "s/"stake"/"axpla"/g" ~/.xpla/config/genesis.json
    ```

11. LCDClient 환경과 RPC 네트워크 연결을 위해 `app.toml` 파일과 `config.toml` 파일을 수정해줍니다. 
    ...
    LCDClient 환경은 1317 Port를 사용하고, RPC 네트워크는 26657 Port를 사용합니다. 앞서 4번 절차에서 Docker Container를 생성할 때, 1317과 26657 Port를 열어주었기 때문에 로컬 환경에서 Docker Container에 접속할 수 있게 됩니다.

12. 지금까지 설정한 Genesis 파일 상태대로 블록체인 데이터베이스를 세팅해줍니다.

    ```bash
    xplad tendermint unsafe-reset-all
    ```

13. 아래 스크립트로 XPLA 블록체인 Local Network를 Background Process로 실행합니다.
...
14. Local Network 구축이 완료되었습니다! 아래 스크립트를 통해 실행되고 있는 Local Network의 정보를 확인할 수 있습니다.
    ...
    이때 **SyncInfo.latest_block_height** 값을 통해, 가장 최신 블록의 Height를 알 수 있습니다. `xplad status` 스크립트를 실행할 때마다, height 값이 상승할 것입니다. 블록체인에 블록이 잘 쌓이고 있다는 의미로 해석할 수 있습니다.
...
1. 잔액 확인
    ...
    위 스크립트를 입력하면 아래와 같이 결과를 확인할 수 있습니다.
...
2. Delegate 물량 확인
    ...
    아래와 같은 결과를 확인할 수 있습니다. Self-delegate까지 되었군요.
...
4. 트랜잭션 Query 보내기
    ...
    xplad로 블록체인에 query를 보내서 앞서 생성한 트랜잭션 데이터를 가져와봅시다. 앞서 생성한 트랜잭션 해시값을 대입하면, 예제 스크립트는 `xplad query tx F8B522E28918C6616136C1B1C70D3B5972F06CA8E80D9BD60B6B19704BC85E5C` 입니다. 여러분이 생성한 트랜잭션 해시값을 대입해주세요. 

    ```bash
    xplad query tx <txhash>
    ```

지금까지 xplad를 활용하여 Local Network 블록체인이 잘 동작하는 것을 확인하였습니다. 더 많은 xplad 명령어는 [Docs](https://docs.xpla.io/docs/develop/tools/xplad/commands/)를 참고해보세요.     
...
자바스크립트 파일 실행만으로는 아직 아무 결과를 얻을 수 없습니다. 다시 Docker Container의 bash 환경으로 돌아가, 전송 트랜잭션을 생성해보세요. 블록체인에 트랜잭션이 생성되면, 자바스크립트 파일을 실행한 콘솔에서 아래 로그를 얻을 수 있습니다.
...
<div className="flex justify-center" style={{backgroundColor : 'hsl(240, 20%, 97%)', position : 'relative'}}>
    <VaultImgCard style={{zIndex : 1, maxWidth : "100%"}} imgSrc="/xpla-academy-dev/img/3-tutorial/3-add-func-to-game-2/1-local-network/swagger.png" />
</div>
</BrowserWindow>
...
위 코드는 [이전 단계](/docs/settings/create-testnet-transaction/send-tx)에서 테스트넷에 전송 트랜잭션을 생성하는 코드와 크게 다르지 않습니다. 달라진 점은 LCDClient의 초기값입니다. chainID와 URL 값을 Local Network에 맞게 변경해주었을 뿐입니다. 따라서 지금까지 Javascript로 진행한 모든 코드들도 LCDClient 초기값만 수정하면 동일하게 Local Network에서 수행할 수 있습니다.
