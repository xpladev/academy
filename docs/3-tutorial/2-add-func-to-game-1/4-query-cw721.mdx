...
게임에서 유저들이 소유한 NFT를 어떻게 이용할 수 있을까요? LCDClient를 이용하여 Query를 보내서 NFT 데이터를 가져올 수 있습니다. [이전 단계](/docs/tutorial/add-func-to-game-1/make-nft-vault)에서 직접 만든 NFT 정보를 가져와봅시다. 예제에서 사용한 CW721 컨트랙트 주소는 `xpla1wx3rm4qxf7l3tczj20mxz62wpnr74kme3f45tvk3muh78c432ucs2ceuqn` 입니다.

## Preview {#preview}

Javascript로 NFT 데이터를 가져오는 코드는 아래와 같습니다. 9번째 줄에 여러분이 만든 CW721 컨트랙트 주소를, 10번째 줄에 NFT를 보유하고 있는 지갑 주소로 수정해주세요. 마지막으로 11번째 줄을 여러분이 만든 NFT의 `token_id` 값으로 수정하고 실행해보세요. 조금만 기다리시면 우측에서 실행 결과를 볼 수 있습니다.
...
<div className="flex justify-center" style={{backgroundColor : 'hsl(240, 20%, 97%)', position : 'relative'}}>
    <VaultImgCard style={{zIndex : 1, maxWidth : "100%"}} imgSrc="/xpla-academy-dev/img/3-tutorial/2-add-func-to-game-1/query-cw721/explorer-initmsg.png" />
</div>
</BrowserWindow>
...
<BrowserWindow url="https://explorer.xpla.io/testnet/address/xpla1xljvdrtyn86kv7hrdhae4qxdy8krajah3w7xhtyrt0n69und9xdqdhrasc#InitMsg" bodyStyle={{padding: 0}}>
<div className="flex justify-center" style={{backgroundColor : 'hsl(240, 20%, 97%)', position : 'relative'}}>
    <VaultImgCard style={{zIndex : 1, maxWidth : "100%"}} imgSrc="/xpla-academy-dev/img/3-tutorial/2-add-func-to-game-1/query-cw721/query.png" />
</div>
</BrowserWindow>
...
이렇게 컨트랙트에 Query를 보내는 방법과 Query 메시지를 작성하는 방법에 대해 알아보았습니다. 이제 Preview Code를 분석해보겠습니다. Preview Code를 이해하셨다면, [아래 내용](/docs/tutorial/add-func-to-game-1/query-cw721#runjs)을 읽지 않고 바로 [다음 단계](/docs/settings/create-testnet-transaction/use-xplad)로 넘어가셔도 좋습니다.

## Javascript로 NFT 데이터 가져오기 {#runjs}

Javascript 코드로 특정 지갑 주소가 어떤 NFT들을 가지고 있는지를 알아보고, 특정 NFT의 `token_id` 값으로 NFT 정보까지 불러와 봅시다.
...
3. 9번째 줄에 여러분이 만든 CW721 컨트랙트 주소를, 10번째 줄에 NFT를 보유하고 있는 지갑 주소로 수정해주세요. 마지막으로 11번째 줄을 여러분이 만든 NFT의 `token_id` 값으로 수정하고 실행해보세요. 
...
다음으로 특정 **token_id**에 해당하는 NFT의 정보를 알아내기 위해, nftInfoMsg 변수에 아래와 같이 Query 메시지를 담아주었습니다. Query 메시지 형식은 [CW721 Github](https://github.com/CosmWasm/cw-nfts/blob/main/contracts/cw721-base/src/msg.rs#L126)을 참고했습니다.
