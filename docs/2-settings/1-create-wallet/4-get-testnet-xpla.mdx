...
The mainnet and testnet even have different names (chainIDs)! The chainID for the mainnet is `dimension_37-1`, while the testnet's chainID is `cube_47-5`. Some of you may have already caught onto this in the [previous step](/docs/settings/create-wallet/create-wallet-with-js#runjs).
...
Have you successfully received the testnet tokens? In the [next step](/docs/settings/create-testnet-transaction/send-tx), we'll utilize the received $XPLA to leave a mark in the blockchain by storing data.
